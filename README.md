...
*   **Phase 10: The Immortal Swarm & Apple Music Database (✅ Active):**
    ...
    *   **Hybrid Intelligent URL Router:** A collaborative frontend/backend feature that parses Apple Music URLs on the fly (e.g., detecting `?i=` or `/artist/`), predicts the user's intended scope, and maps it to explicit CLI arguments. This prevents catastrophic bandwidth waste from erroneous full-album downloads while preserving the user's ability to manually override decisions via the UI.
