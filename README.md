...
*   **Phase 13: Architectural Polish & Reliability (✅ Active):**
    *   **Global Persistent Target Routing:** The Swarm Command Center now uses a unified `targetWorkerId` managed in a globally persisted Zustand store (`swarm-fleet-store`). Selecting a Node in the Fleet Bar dynamically routes all subsequent Apple Music tasks, Chat messages, File Explorer queries, and VFS Indexing jobs to that exact node, persisting seamlessly across browser tab reloads.
    *   **Queue Ledger Persistence:** The Apple Music Distributed Job Ledger (`swarm-apple-music-queue`) natively persists across browser sessions. Progress bars, completed task telemetry, and Zero-Disk Handoff states survive hard refreshes natively using Zustand middleware.
    *   **Smart VFS Path Locking:** The UI intelligently manages background `rclone` polling. The `MiniBrowser` utilized for Zero-Disk Cloud Handoff configuration locks selected cloud target paths into a static state, actively preventing WebWorker payload spam and avoiding backend `rclone` daemon crashes when dealing with massive file trees.
    *   **VFS Database Mock Fallback:** To ensure development stability, if a backend ephemeral worker attempts to bypass the Relay Server to build a massive VFS Index but the targeted database engine is offline (or defaulting to MOCK), the worker gracefully catches the `ECONNREFUSED` error. It simulates the ingestion of the 10GB payload natively in RAM and reports a simulated `SUCCESS` back to the UI rather than crashing the execution environment.
    *   **Dynamic Config Handoff:** Ephemeral backend workers accurately identify their dynamically constructed `/tmp/rclone.conf` files, passing explicit `--config` arguments during massive server-side copy/move operations to guarantee multi-cloud data operations succeed.
    *   **Intelligent Auto-Fill Configs:** The VFS Engine UI parses raw `rclone.conf` text block payloads pasted by users, utilizing Regex to extract the internal `[remote_name]` bracket and automatically formatting and binding it to the Physical Rclone Name field with the required trailing colon.
