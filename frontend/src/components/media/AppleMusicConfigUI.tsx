...
export const AppleMusicConfigUI: React.FC = () => {
  ...
  // Local state for splitting the combined `rcloneRemote` (e.g., 'gdrive:/Music/Rips')
  const [selectedFs, setSelectedFs] = useState<string>('');
  const [remotePath, setRemotePath] = useState<string>('');
  const [availableRemotes, setAvailableRemotes] = useState<string[]>([]);

  useEffect(() => {
     // Fetch remotes for the dropdown
     const socketManager = SocketManager.getInstance();

     const handleRemoteList = (msg: any) => {
        if (msg.remotes) {
           const remoteNames = msg.remotes.map((r: any) => r.name);
           setAvailableRemotes(remoteNames);

           // If we don't have a selected FS but we do have remotes, set a default
           if (!selectedFs && remoteNames.length > 0 && rcloneRemote === 'remote:/Media/AppleMusic_Rips') {
              setSelectedFs(remoteNames[0] === '/' && remoteNames.length > 1 ? remoteNames[1] : remoteNames[0]);
           }
        }
     };

     socketManager.on(MessageType.REMOTE_LIST_RESPONSE, handleRemoteList);
     // We ask 'target-worker-id' or the first available worker for its remotes
     socketManager.emit(MessageType.REMOTE_LIST_REQUEST, { timestamp: Date.now(), workerId: '*' });

     return () => {
        socketManager.off(MessageType.REMOTE_LIST_RESPONSE, handleRemoteList);
     };
  }, []);
  ...
  useEffect(() => {
     if (rcloneRemote) {
        ...
        if (parts.length > 1) {
           setSelectedFs(parts[0] + ':');
           setRemotePath(parts.slice(1).join(':'));
        } else {
           setSelectedFs('/');
    ...
    <div className="w-full h-full flex flex-col items-center py-10 overflow-y-auto">
      <div className="w-full max-w-4xl px-6 space-y-6">
        ...
        <div className="p-8 bg-black/40 border border-white/10 rounded-2xl backdrop-blur-sm space-y-6">
            ...
            {/* Zero-Disk Cloud Handoff Settings */}
            <h4 className="text-sm uppercase tracking-wider text-muted-foreground font-bold border-b border-white/10 pb-2 mt-8 flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-blue-500 animate-pulse"></span>
                Zero-Disk Cloud Handoff & Storage Routing
            </h4>
            <div className="p-5 bg-[#050505] border border-white/5 rounded-xl space-y-5">
                <div className="flex items-center justify-between">
                    <div>
                        <h5 className="text-sm text-white font-semibold">Enable Automatic Cloud Upload</h5>
                        <p className="text-[10px] text-muted-foreground mt-0.5">When enabled, downloaded media is securely moved to the selected Remote and immediately purged from the ephemeral worker to preserve disk space.</p>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" className="sr-only peer" checked={autoUpload} onChange={(e) => useAppleMusicStore.getState().setAutoUpload(e.target.checked)} />
                ...
                <div className={`grid grid-cols-2 gap-6 transition-opacity duration-300 ${!autoUpload ? 'opacity-30 pointer-events-none' : ''}`}>
                    <div className="space-y-1">
                        ...
                        >
                            {availableRemotes.map(remote => (
                                <option key={remote} value={remote}>{remote === '/' ? 'Local Ephemeral Disk (/)' : remote}</option>
                            ))}
                            {availableRemotes.length === 0 && <option value="remote:">remote: (Fallback)</option>}
                        </select>
                    </div>
                    <div className="space-y-1">
                        <label className="text-xs text-muted-foreground ml-1 font-semibold tracking-wide">Destination Folder Path</label>
                        <input
                            type="text"
                            value={remotePath}
                ...
                <div className={`mt-2 flex items-center gap-2 px-1 text-[10px] text-muted-foreground ${!autoUpload ? 'opacity-30' : ''}`}>
                    <span className="font-semibold text-blue-400">Final Target:</span>
                    <span className="truncate font-mono bg-white/5 px-2 py-0.5 rounded border border-white/10">
                        {rcloneRemote}
                    </span>
                </div>
            </div>
